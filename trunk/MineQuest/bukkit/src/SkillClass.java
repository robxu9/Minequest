...
public class SkillClass {
	...
	public SkillClass(Quester quester, String type, String name, mysql_interface sql) {
		this.name = name;
		sql_server = sql;
		this.type = type;
...
//		}
	...
	public int defend(Quester quester, LivingEntity entity, int amount) {
		...
		for (i = 0; i < ability_list.length; i++) {
			if (ability_list[i].isDefending(quester, entity)) {
...
//		}
	...
	public void save() {
		sql_server.update("UPDATE classes SET exp='" + exp + "', level='" + level + 
	...
	public void update() {
		...
		results = sql_server.query("SELECT * FROM classes WHERE name='" + name + "' AND class='" + type + "'");
