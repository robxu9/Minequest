...
public class Town {
	...
	public void finishStore(Player player, String name) {
		if (town.canEdit(MineQuest.getQuester(player))) {
			...
			sql_server.update("INSERT INTO " + this.name + 
					" (name, x, y, z, max_x, max_z, height, store_prop, price) VALUES('"
					+ name + "', '" + x + "', '" + y + "', '" + z
					+ "', '" + max_x + "', '" + max_z + "', '" + height
					+ "', '1', '10000000')");
			sql_server.update("CREATE TABLE IF NOT EXISTS " + name + " (item_id INT, price DOUBLE, quantity INT, type VARCHAR(30))");
			stores.add(new Store(name, sql_server, this.name));
	...
	public void setSpawn(Location spawn) {
		this.spawn = spawn;
		sql_server.update("UPDATE towns SET spawn_x='" + (int)spawn.getX() + "', spawn_y='" + 
